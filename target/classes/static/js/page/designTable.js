...
function opttable() {
    //给每一个单元格添加对应的事件
    $('#optfile').find('form').find('td').each(function () {
        $(this).mousedown(function (e) {//鼠标按键按下
            ...
            if (tabledata.opt) {
                ...
                /*if(tabledata.previousElement==null) {
                        console.log('第一次');
                        setSelectTd().col(opt);
                        setSelectTd().row(opt);
                }else{
                    if(tabledata.previousElement.col>=tabledata.downElement.col&&opt.col>tabledata.previousElement.col){
                        setSelectTd().row(opt);
                    }else if(tabledata.previousElement.col<=tabledata.downElement.col&&opt.col<tabledata.previousElement.col){
                        setSelectTd().row(opt);
                    }else{
                        if(tabledata.previousElement.col>tabledata.downElement.col&&tabledata.previousElement.col>opt.col&&tabledata.downElement.col<opt.col){
                            setRemoveTd().row();
                        }else if (tabledata.previousElement.col<tabledata.downElement.col&&tabledata.previousElement.col<opt.col&&tabledata.downElement.col>opt.col) {
                            setRemoveTd().row();
                        }
                    }

                    if(tabledata.previousElement.row>=tabledata.downElement.row&&opt.row>tabledata.previousElement.row){
                        setSelectTd().col(opt);
                    }else if(tabledata.previousElement.row<=tabledata.downElement.row&&opt.row<tabledata.previousElement.row){
                        setSelectTd().col(opt);
                    }else{
                        if(tabledata.previousElement.row>tabledata.downElement.row&&tabledata.previousElement.row>opt.row&&tabledata.downElement.row<opt.row){
                            setRemoveTd().col();
                        }else if (tabledata.previousElement.row<tabledata.downElement.row&&tabledata.previousElement.row<opt.row&&tabledata.downElement.row>opt.row) {
                            setRemoveTd().col();
                        }
                    }
